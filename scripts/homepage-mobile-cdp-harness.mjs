...
async function main() {
  const options = process.argv.slice(2).reduce((acc, arg) => { const [key, value] = arg.split(/=(.*)/s); if (key === "--output-dir") acc.outputDir = value; if (key === "--server-port") acc.serverPort = Number(value); if (key === "--cdp-port") acc.cdpPort = Number(value); if (key === "--first-scroll-only") acc.firstScrollOnly = true; return acc; }, { outputDir: OUT, serverPort: 3200, cdpPort: 9222, firstScrollOnly: false }), outputDir = resolveOutputDir(options.outputDir), tempRoot = path.join(outputDir, "temp"), baseUrl = `http://127.0.0.1:${options.serverPort}`, report = { startedAt: new Date().toISOString(), baseUrl, outputDir, commands: [], firstScrollResults: [], firstScrollDiagnostics: [], productionResults: [], exploreMenuResults: [], mobileMenuResults: [], cliProofResults: [], cleanup: [], pass: false };
