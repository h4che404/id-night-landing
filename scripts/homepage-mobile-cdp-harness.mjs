...
const HEADLESS_RENDERING_FLAGS = ["--disable-background-timer-throttling", "--disable-backgrounding-occluded-windows", "--disable-renderer-backgrounding"];
...
export const ACTIVE_SCROLL_PROFILES = [
...
const roundedPercentile = (values, fraction) => { const value = percentile(values.filter(Number.isFinite), fraction); return value === null ? null : +value.toFixed(2); };
export function isActiveScrollPass(result) { const mobile = result.profile?.input === "touch", software = result.profile?.softwareRendering, responseLimit = mobile ? 250 : software ? 120 : 50, frameGapLimit = mobile ? 250 : software ? 300 : 150, resumeLimit = mobile ? 750 : software ? 1400 : 500, resumeDraws = software ? 1 : 4, minimumInputs = mobile ? 2 : 10, longTaskLimit = software ? 20 : mobile ? 3 : 2; return result.samples.length >= 3 && result.summary.responseP50Ms <= responseLimit && result.summary.responseP95Ms <= responseLimit * 1.8 && result.summary.activeFrameGapP95Ms <= frameGapLimit && result.summary.activeLongTasks <= longTaskLimit && result.samples.every((sample) => sample.inputEvents >= minimumInputs && sample.scrollEvents >= 6 && sample.scrollDelta > 200 && sample.activeScrollMs >= 450 && sample.activeCanvasDraws <= 2 && sample.scrollPausedThroughout && sample.activeCanvasPaused && sample.resumeCanvasDraws >= resumeDraws && sample.resumeDelayMs >= 100 && sample.resumeDelayMs <= resumeLimit && sample.canvasScrollOptimized && sample.canvasVisible && sample.canvasDpr <= 1.5); }
...
function summarizeTrace(events) {
  ...
  const durations = (name) => complete.filter((event) => event.name === name).map((event) => event.dur / 1_000);
  const total = (name) => durations(name).reduce((sum, duration) => sum + duration, 0);
  return {
    runTaskMaxMs: +(Math.max(0, ...durations("RunTask"))).toFixed(2),
    paintTotalMs: +total("Paint").toFixed(2),
    rasterTaskTotalMs: +total("RasterTask").toFixed(2),
    compositeLayersTotalMs: +total("CompositeLayers").toFixed(2),
    updateLayerTreeTotalMs: +total("UpdateLayerTree").toFixed(2),
    eventDispatchTotalMs: +total("EventDispatch").toFixed(2),
    functionCallTotalMs: +total("FunctionCall").toFixed(2),
    animationFrameTotalMs: +total("FireAnimationFrame").toFixed(2),
...
async function probeActiveScroll(client, baseUrl, profile, attempts = 3, mode = "normal") {
  const { viewport, deviceScaleFactor, cpuThrottleRate, input } = profile;
  const samples = [];
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    const suppressScrollListeners = mode === "scroll-listeners-disabled" || mode === "observers-listeners-disabled";
    const suppressIntersectionObservers = mode === "intersection-observers-disabled" || mode === "observers-listeners-disabled";
    const attributeScrollListeners = mode === "scroll-listener-attribution";
    const listenerSuppression = suppressScrollListeners || suppressIntersectionObservers || attributeScrollListeners ? await client.send("Page.addScriptToEvaluateOnNewDocument", { source: `(()=>{const nativeAdd=EventTarget.prototype.addEventListener,blocked=${suppressScrollListeners ? "new Set(['scroll'])" : "new Set()"};window.__nativeAddEventListener=nativeAdd;window.__scrollListenerMetrics=[];EventTarget.prototype.addEventListener=function(type,listener,...args){if(blocked.has(type))return;if(type==='scroll'&&${attributeScrollListeners}){const stack=new Error().stack?.split('\\n').slice(2,9).join('\\n')??'unknown',wrapped=function(event){const started=performance.now();try{return typeof listener==='function'?listener.call(this,event):listener.handleEvent(event)}finally{window.__scrollListenerMetrics.push({stack,duration:performance.now()-started,target:this===window?'window':this===document?'document':this?.constructor?.name??'unknown'})}};return nativeAdd.call(this,type,wrapped,...args)}return nativeAdd.call(this,type,listener,...args)};${suppressIntersectionObservers ? "window.IntersectionObserver=class{observe(){}unobserve(){}disconnect(){}takeRecords(){return[]}get root(){return null}get rootMargin(){return '0px'}get thresholds(){return[0]}}" : ""}})()` }) : null;
    if (mode === "reduced-motion") await client.send("Emulation.setEmulatedMedia", { features: [{ name: "prefers-reduced-motion", value: "reduce" }] });
    ...
    if (profile.softwareRendering) await client.eval("window.__scrollWarmup=setInterval(()=>dispatchEvent(new WheelEvent('wheel')),50)");
    await delay(profile.softwareRendering ? 200 : 650);
    if (mode === "canvas-static" || mode === "canvas-bitmap" || mode === "scroll-listeners-disabled" || mode === "intersection-observers-disabled" || mode === "observers-listeners-disabled") {
      ...
      await delay(150);
    } else if (mode === "canvas-hidden") {
      await client.eval(`document.querySelector('#vision canvas').style.visibility='hidden'`);
    } else if (mode === "radial-gradients-disabled") {
      await client.eval(`document.querySelector('#vision > div')?.style.setProperty('display','none')`);
    } else if (mode === "hero-gradients-disabled") {
      await client.eval(`Array.from(document.querySelectorAll('#vision > div')).slice(0,2).forEach(node=>node.style.display='none')`);
    ...
    } else if (mode === "canvas-promoted") {
      await client.eval(`document.querySelector('#vision canvas').style.cssText+=';transform:translateZ(0);will-change:transform'`);
    } else if (mode === "hero-contained") {
      await client.eval(`document.querySelector('#vision').style.contain='paint'`);
    } else if (mode === "pointer-disabled") {
      await client.eval("document.querySelector('#vision').dispatchEvent(new PointerEvent('pointerleave',{bubbles:true}))");
    }
    client.drainEvents();
    ...
    await client.eval(`(()=>{clearInterval(window.__scrollWarmup);const canvas=document.querySelector('#vision canvas'),header=document.querySelector('header');window.__activeScrollProbe={startedAt:performance.now(),inputAt:null,lastInputAt:null,lastScrollAt:null,inputEvents:0,inputDraws:null,activeEndedAt:null,activeEndDraws:null,activePaused:false,frames:[],scrolls:[],headerMutations:[],longTasks:[],startY:scrollY};const probe=window.__activeScrollProbe,listen=window.__nativeAddEventListener?.bind(window)??addEventListener.bind(window);const read=()=>({time:performance.now(),draws:Number(canvas?.dataset.drawCount||0),paused:canvas?.dataset.scrollPaused==='true'});const frame=time=>{probe.frames.push({...read(),time});if(time-probe.startedAt<6000)requestAnimationFrame(frame)};const markInput=()=>{const state=read();probe.inputAt??=state.time;probe.inputDraws??=state.draws;probe.lastInputAt=state.time;probe.inputEvents+=1};requestAnimationFrame(frame);listen('wheel',markInput,{passive:true});listen('touchstart',markInput,{passive:true});listen('touchmove',markInput,{passive:true});listen('scroll',()=>{const state=read();probe.lastScrollAt=state.time;probe.scrolls.push({...state,y:scrollY})},{passive:true});new MutationObserver(()=>probe.headerMutations.push({time:performance.now(),className:header.className})).observe(header,{attributes:true,attributeFilter:['class']});new PerformanceObserver(list=>probe.longTasks.push(...list.getEntries().map(entry=>({startTime:entry.startTime,duration:entry.duration})))).observe({type:'longtask',buffered:true})})()`);
    if (input === "touch") {
      ...
      for (const offset of [25, 50, 75, 100, 125, 150, 175, 200, 225]) {
        await delay(60);
      ...
      await client.eval("new Promise(resolve=>{let steps=0;const inertia=setInterval(()=>{scrollBy(0,8);if(++steps===3){clearInterval(inertia);resolve()}},60)})");
    } else {
      await client.send("Input.dispatchMouseEvent", { type: "mouseWheel", x: Math.round(viewport.width / 2), y: Math.min(320, viewport.height - 1), deltaX: 0, deltaY: 36 });
      await client.eval("new Promise(resolve=>{let steps=0;const sustained=setInterval(()=>{dispatchEvent(new WheelEvent('wheel',{deltaY:36}));scrollBy(0,36);if(++steps===9){clearInterval(sustained);resolve()}},60)})");
    }
    await client.eval(`(()=>{const probe=window.__activeScrollProbe,canvas=document.querySelector('#vision canvas');probe.activeEndedAt=performance.now();probe.activeEndDraws=Number(canvas?.dataset.drawCount||0);probe.activePaused=canvas?.dataset.scrollPaused==='true'})()`);
    await delay(profile.softwareRendering ? 1400 : 700);
    ...
    const sample = await client.eval(`(()=>{const probe=window.__activeScrollProbe,canvas=document.querySelector('#vision canvas'),lastActivityAt=Math.max(probe.lastInputAt??0,probe.lastScrollAt??0),activeFrames=probe.frames.filter(frame=>frame.time>=probe.inputAt&&frame.time<=probe.activeEndedAt),activeFrameGaps=activeFrames.slice(1).map((frame,index)=>frame.time-activeFrames[index].time),firstScroll=probe.scrolls[0],endDraws=Number(canvas?.dataset.drawCount||0),style=getComputedStyle(canvas),resumeFrame=probe.frames.find(frame=>frame.time>probe.activeEndedAt&&frame.draws>probe.activeEndDraws&&!frame.paused),activeLongTasks=probe.longTasks.filter(task=>task.startTime>=probe.inputAt&&task.startTime<=probe.activeEndedAt);return{responseMs:firstScroll&&probe.inputAt!==null?firstScroll.time-probe.inputAt:null,activeScrollMs:lastActivityAt-probe.inputAt,inputEvents:probe.inputEvents,activeFrameGaps,scrollEvents:probe.scrolls.length,scrollDelta:scrollY-probe.startY,activeCanvasDraws:probe.activeEndDraws-probe.inputDraws,activeCanvasPaused:probe.activePaused,scrollPausedThroughout:probe.scrolls.every(entry=>entry.paused),resumeCanvasDraws:endDraws-probe.activeEndDraws,resumeDelayMs:resumeFrame?resumeFrame.time-lastActivityAt:null,canvasScrollPaused:canvas?.dataset.scrollPaused==='true',canvasScrollOptimized:canvas?.dataset.scrollOptimized==='true',canvasRafActive:canvas?.dataset.rafActive,canvasVisible:style.display!=='none'&&style.visibility!=='hidden'&&Number(style.opacity)>0,canvasDpr:Number(canvas?.dataset.dpr),devicePixelRatio,pointCount:Number(canvas?.dataset.pointCount),listenerMetrics:window.__scrollListenerMetrics??[],activeLongTasks,longTasks:probe.longTasks}})()`);
    samples.push({ attempt: attempt + 1, ...sample, trace: attempt === 0 ? summarizeTrace(traceEvents) : undefined });
    if (listenerSuppression) await client.send("Page.removeScriptToEvaluateOnNewDocument", { identifier: listenerSuppression.identifier });
    if (mode === "reduced-motion") await client.send("Emulation.setEmulatedMedia", { features: [] });
  }
  const summary = {
    responseP50Ms: roundedPercentile(samples.map((sample) => sample.responseMs), 0.5),
    responseP95Ms: roundedPercentile(samples.map((sample) => sample.responseMs), 0.95),
    activeFrameGapP95Ms: roundedPercentile(samples.flatMap((sample) => sample.activeFrameGaps), 0.95),
    activeLongTasks: samples.reduce((total, sample) => total + sample.activeLongTasks.length, 0),
  };
...
async function probeExploreMenu(client, baseUrl, outputDir, viewport) {
  ...
  await waitFor(() => client.eval(`location.hash==="#problema"&&!document.querySelector('#site-explore-menu')`), `desktop menu hash closure at ${fmt(viewport)}`, 2_000);
...
async function probeMobileMenu(client, baseUrl, outputDir, viewport) {
  ...
  await waitFor(() => client.eval(`!document.querySelector('#site-mobile-menu')&&document.activeElement?.getAttribute?.('aria-controls')==="site-mobile-menu"&&document.body.style.overflow===""`), `mobile menu Escape closure at ${fmt(viewport)}`, 2_000);
...
async function probeNeuralBackground(client, baseUrl) {
  ...
  await delay(600);
  client.drainEvents();
  const snapshot = () => client.eval(`(()=>{const canvas=document.querySelector('#vision canvas');return{sampledAt:performance.now(),pointCount:Number(canvas?.dataset.pointCount),drawCount:Number(canvas?.dataset.drawCount),rafRequests:Number(canvas?.dataset.rafRequests),rawRafCount:Number(window.__rawRafCount||0),rafActive:canvas?.dataset.rafActive,pointerActive:canvas?.dataset.pointerActive,dpr:Number(canvas?.dataset.dpr)}})()`);
  ...
  for (const viewport of NEURAL_VIEWPORTS.filter(({ width }) => width === 375 || width === 1440)) {
    await setViewport(viewport);
    await client.eval(`(()=>{window.__rawRafCount=0;window.__rawRafActive=true;requestAnimationFrame(function tick(){window.__rawRafCount+=1;if(window.__rawRafActive)requestAnimationFrame(tick)})})()`);
    await delay(350);
    const start = await snapshot();
    await delay(1_200);
    const end = await snapshot();
    await client.eval(`window.__rawRafActive=false`);
    const elapsedMs = end.sampledAt - start.sampledAt;
    const drawDelta = end.drawCount - start.drawCount;
    const rawRafDelta = end.rawRafCount - start.rawRafCount;
    visibleSamples.push({ viewport, start, end, elapsedMs, drawDelta, rawRafDelta, cadence: drawDelta * 1_000 / elapsedMs, sourceCadence: rawRafDelta * 1_000 / elapsedMs });
  }
...
export function isNeuralBackgroundPass(result) {
  ...
  const cadencePass = visibleSamples.length === 2 && visibleSamples.every(({ cadence, sourceCadence, drawDelta, rawRafDelta, start, end }) => { const targetCadence = Math.min(60, sourceCadence); return sourceCadence >= 8 && cadence >= targetCadence * 0.8 && cadence <= targetCadence * 1.1 && drawDelta >= 10 && rawRafDelta >= drawDelta && start.rafActive === "true" && end.rafActive === "true"; });
  return densityPass && cadencePass && interactive.pointCount >= 52 && interactive.pointCount <= 72 && interactive.pointerActive === "true" && interactive.rafActive === "true" && interactive.dpr <= 1.5 && offscreenStart.rafActive === "false" && offscreenEnd.rafRequests === offscreenStart.rafRequests && offscreenEnd.drawCount === offscreenStart.drawCount && resumedStart.rafActive === "true" && resumedRequests >= resumedDraws && resumedRequests <= resumedDraws * 3 + 3 && resumedDraws >= 3 && resumedDraws <= 24 && reducedStart.rafActive === "false" && reducedEnd.rafRequests === reducedStart.rafRequests && reducedEnd.drawCount === reducedStart.drawCount && reducedEnd.pointerActive === "false" && runtimeEvents.length === 0;
}
...
async function main() {
  const options = process.argv.slice(2).reduce((acc, arg) => { const [key, value] = arg.split(/=(.*)/s); if (key === "--output-dir") acc.outputDir = value; if (key === "--server-port") acc.serverPort = Number(value); if (key === "--cdp-port") acc.cdpPort = Number(value); if (key === "--active-scroll-only" || key === "--first-scroll-only") acc.activeScrollOnly = true; if (key === "--performance-only") { acc.performanceOnly = true; acc.activeScrollOnly = true; } if (key === "--mobile-causality-only") { acc.mobileCausalityOnly = true; acc.activeScrollOnly = true; } if (key === "--mobile-modes") acc.mobileModes = value?.split(",").filter(Boolean); return acc; }, { outputDir: OUT, serverPort: 3200, cdpPort: 9222, activeScrollOnly: false, performanceOnly: false, mobileCausalityOnly: false, mobileModes: null }), outputDir = resolveOutputDir(options.outputDir), tempRoot = path.join(outputDir, "temp"), baseUrl = `http://127.0.0.1:${options.serverPort}`, report = { startedAt: new Date().toISOString(), baseUrl, outputDir, commands: [], activeScrollResults: [], activeScrollDiagnostics: [], productionResults: [], exploreMenuResults: [], mobileMenuResults: [], cliProofResults: [], cleanup: [], pass: false };
  ...
  let softwareBrowser, softwareClient;
  try {
    ...
    browser = launch(chromePath, ["--headless=new", ...HEADLESS_RENDERING_FLAGS, `--remote-debugging-port=${options.cdpPort}`, `--user-data-dir=${path.join(tempRoot, "chrome-cdp-profile")}`, "about:blank"], path.join(outputDir, `chrome-cdp-${options.cdpPort}.log`)); await waitFor(() => httpJson(`http://127.0.0.1:${options.cdpPort}/json/version`), `Chrome debugger on ${options.cdpPort}`); client = await cdp(options.cdpPort); await client.send("Page.enable"); await client.send("Runtime.enable"); report.cleanup.push(`chrome cdp pid=${browser.child.pid}`);
    if (options.mobileCausalityOnly) {
      report.activeScrollResults.push(await probeActiveScroll(client, baseUrl, ACTIVE_SCROLL_PROFILES[0]));
      const mobileModes = options.mobileModes ?? MOBILE_COMPOSITING_MODES.filter((entry) => entry !== "normal");
      for (const mode of mobileModes) {
        if (!MOBILE_COMPOSITING_MODES.includes(mode) || mode === "normal") throw new Error(`Unknown mobile diagnostic mode: ${mode}`);
        report.activeScrollDiagnostics.push(await probeActiveScroll(client, baseUrl, ACTIVE_SCROLL_PROFILES[0], 3, mode));
      }
      report.pass = report.activeScrollResults.every(isActiveScrollPass);
      return;
    }
    for (const profile of ACTIVE_SCROLL_PROFILES.filter((entry) => !entry.softwareRendering)) report.activeScrollResults.push(await probeActiveScroll(client, baseUrl, profile));
    const softwarePort = options.cdpPort + 20;
    softwareBrowser = launch(chromePath, ["--headless=new", "--disable-gpu", ...HEADLESS_RENDERING_FLAGS, `--remote-debugging-port=${softwarePort}`, `--user-data-dir=${path.join(tempRoot, "chrome-software-profile")}`, "about:blank"], path.join(outputDir, `chrome-software-${softwarePort}.log`));
    await waitFor(() => httpJson(`http://127.0.0.1:${softwarePort}/json/version`), `software Chrome debugger on ${softwarePort}`); softwareClient = await cdp(softwarePort); await softwareClient.send("Page.enable"); await softwareClient.send("Runtime.enable"); report.cleanup.push(`software chrome pid=${softwareBrowser.child.pid}`);
    for (const profile of ACTIVE_SCROLL_PROFILES.filter((entry) => entry.softwareRendering)) report.activeScrollResults.push(await probeActiveScroll(softwareClient, baseUrl, profile));
    if (options.activeScrollOnly) {
      if (!options.performanceOnly) for (const mode of ["canvas-static", "glow-disabled", "pointer-disabled", "navbar-effects-disabled"]) report.activeScrollDiagnostics.push(await probeActiveScroll(softwareClient, baseUrl, ACTIVE_SCROLL_PROFILES[2], 3, mode));
      report.pass = report.activeScrollResults.every(isActiveScrollPass);
      return;
    }
    await softwareClient.close(); softwareClient = undefined;
    const softwareProfileCleanup = await stop(softwareBrowser); softwareBrowser = undefined;
    report.cleanup.push(`software profile terminated=${softwareProfileCleanup.terminated} forced=${softwareProfileCleanup.forced}`);
    ...
    report.pass = report.activeScrollResults.every(isActiveScrollPass) && report.productionResults.every(isProductionViewportPass) && report.exploreMenuResults.every(isExploreMenuPass) && report.mobileMenuResults.every(isMobileMenuPass) && isNeuralBackgroundPass(report.neuralBackground) && isPinnedNarrativePass(report.pinnedNarrative) && report.cliProofResults.every((r) => r.cropDetected && r.metrics.innerWidth !== r.viewport.width && Math.round(r.metrics.visualViewportWidth ?? -1) !== r.viewport.width);
  } catch (error) { report.error = error instanceof Error ? error.message : String(error); } finally {
    unbindSignals(); await softwareClient?.close().catch(() => undefined); await client?.close().catch(() => undefined); const softwareCleanup = await stop(softwareBrowser), browserCleanup = await stop(browser), serverCleanup = await stop(server); fs.rmSync(tempRoot, { recursive: true, force: true }); report.cleanup.push(`software chrome terminated=${softwareCleanup.terminated} forced=${softwareCleanup.forced}`, `chrome terminated=${browserCleanup.terminated} forced=${browserCleanup.forced}`, `server terminated=${serverCleanup.terminated} forced=${serverCleanup.forced}`, `temp removed=${tempRoot}`); fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`); fs.writeFileSync(summaryPath, summary(report), "utf8");
  }
